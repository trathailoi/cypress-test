  ...
  parameters {
	...
	stages {
      ...
      parallel {
        stage('Chrome') {
            steps{
                sh "CYPRESS_baseUrl=$siteURL npm run cy:chrome" // -- --parallel --env coverage=true --config video=true
            }
        }
        // stage('Firefox') {
        //     steps{
        ...
        //     steps{
        //         publishHTML([allowMissing: false, alwaysLinkToLastBuild: false, keepAll: false, reportDir: 'cypress/reports/mochareports', reportFiles: 'report.html', reportName: "E2E Report - $siteURL", reportTitles: "$siteURL"])
        //     }
		// }
	}
  post{
